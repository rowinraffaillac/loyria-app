    ...
    <AppShell property={property}>
      <div className="px-8 py-8">
          ...
          <div className="bg-[#13131a] border border-white/8 rounded-xl p-6">
            ...
            <dl className="space-y-2.5">
              ...
              <Row
                ...
                value={property.condo_charges ? `${fmt(property.condo_charges * 12)} / an (${fmt(property.condo_charges)} / mois)` : null}
              />
            </dl>
          </div>
        ...
        <div className="mt-6 grid grid-cols-2 lg:grid-cols-4 gap-4">
          {['Locataire', 'Finances', 'Travaux', 'Documents'].map(module => (
            <div key={module} className="bg-[#13131a]/50 border border-white/5 rounded-xl p-4 text-center">
              <p className="text-sm text-gray-600">{module}</p>
              <p className="text-xs text-gray-700 mt-1">Bientôt disponible</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-8 pt-6 border-t border-white/5">
          <form action={async () => { 'use server'; await archiveProperty(id) }}>
            <button
              ...
              onClick={(e) => {
                if (!confirm('Archiver ce bien ? Il ne sera plus visible dans votre liste.')) {
                  e.preventDefault()
                }
              }}
            >
